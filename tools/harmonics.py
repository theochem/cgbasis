...
from sympy import sqrt, Symbol, pi, S, Add, cos, sin, Wild, latex
...
def get_cart_orb_norm(alpha, nx, ny, nz):
    """Returns the norm of a Cartesian gaussian function"""
    return sqrt(
...
def get_pure_orb_norm(alpha, l):
    """Returns the norm of a pure gaussian function"""
    return sqrt(
        int(fac2(2 * l - 1))
        / (2 * alpha / pi) ** (3 / S(2))
        / (4 * alpha) ** l
...
def get_solid_harmonics(l, r, x, y, z):
    def get_pi(l, m):
        ...
        for k in range((l - m) / 2 + 1):
            factor = (-1) ** k * int(
                binomial(l, k) * binomial(2 * l - 2 * k, l) * fac(l - 2 * k)) / S(
                2 ** l * int(fac(l - 2 * k - m)))
