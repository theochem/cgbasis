...
from gbasis.gobasis import go_basis_families_list
from gbasis.periodic import num2sym
...
for family in go_basis_families_list:
    ...
    for i in range(len(numbers)):
        if (0 < i < len(numbers) - 1) and numbers[i - 1] == numbers[i] - 1 \
                and numbers[i + 1] == numbers[i] + 1:
            if s[-1] != '-':
                s.append('-')
        else:
            s.append(num2sym[numbers[i]])
